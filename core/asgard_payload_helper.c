...
char *asgard_get_proto(struct asgard_payload *spay, int n)
{
	...
	if (spay->protocols_included < n) {
		asgard_error("only %d protocols are included, requested %d", spay->protocols_included, n);
		return NULL;
	}
...
EXPORT_SYMBOL(asgard_get_proto);
...
char *asgard_reserve_proto(u16 instance_id, struct asgard_payload *spay, u16 proto_size)
{
	...
	if (unlikely(proto_offset + proto_size > MAX_ASGARD_PAYLOAD_BYTES)) {
		asgard_error("Not enough space in asgard payload\n");
		spay->protocols_included--;
		return NULL;
	}
...
EXPORT_SYMBOL(asgard_reserve_proto);
...
/* Must be called after the asgard packet has been emitted.
 */
...
int _do_prepare_log_replication(struct asgard_device *sdev, int target_id, s32 next_index, int retrans)
{
    ...
    for (j = 0; j < sdev->num_of_proto_instances; j++) {
        ...
        if (sdev->protos[target_id] != NULL && sdev->protos[j]->proto_type == ASGARD_PROTO_CONSENSUS) {
...
int _do_prepare_log_replication_multicast(struct asgard_device *sdev, u32 dst_ip, unsigned char *dst_mac){
    int ret;
...
void _schedule_update_from_userspace(struct asgard_device *sdev, struct synbuf_device *syndev) {
    ...
    struct asgard_ringbuf_read_work_data *work = NULL;

    if(!sdev->is_leader || sdev->pminfo.state != ASGARD_PM_EMITTING)
        return;

    // freed by pull_consensus_requests_from_rb
    work = kmalloc(sizeof(struct asgard_ringbuf_read_work_data), GFP_KERNEL);

    if(!work) {
        asgard_error("Could not allocate memory for work item in %s\n", __FUNCTION__);
        return;
    }
    ...
    //if(!queue_delayed_work(sdev->asgard_ringbuf_reader_wq, &work->dwork, 0)) {
 ...
 */
void pull_consensus_requests_from_rb(struct work_struct *w) {
    struct asgard_ringbuf_read_work_data *aw = NULL;
    struct consensus_priv *priv;
    int cur_nxt_idx;
    u8 num_of_chunks, i;
    struct data_chunk *new_chunk;
    int err = 0;
    struct asgard_ringbuf_read_work_data *next_work = NULL;
    struct delayed_work *dw;

    dw = container_of(w, struct delayed_work, work);
    aw = container_of(dw, struct asgard_ringbuf_read_work_data, dwork);
    ...
    while(1) {
        ...
        for (i = 0; i < num_of_chunks + 1; i++) {
            ...
            if(err) {
                kfree(new_chunk);
                asgard_dbg("Failed to read from ring buffer\n");

                break;
            }
           // asgard_dbg("asgObj append %u of %u chunks\n", i, num_of_chunks);
            ...
            if(err) {
                asgard_error("Failed to append new chunk to ASGARD log\n");
                break;
            }
            cur_nxt_idx++;
        }
...
cleanup:
    ...
    if(aw->sdev->is_leader && aw->sdev->pminfo.state == ASGARD_PM_EMITTING) {
        // freed by pull_consensus_requests_from_rb
        next_work = kmalloc(sizeof(struct asgard_ringbuf_read_work_data), GFP_KERNEL);
       ...
       // queue_delayed_work(aw->sdev->asgard_ringbuf_reader_wq, &next_work->dwork, 5);
    }
...
void _schedule_log_rep(struct asgard_device *sdev, int target_id, int next_index, s32 retrans, int multicast_enabled)
{
    ...
    //prepare_log_replication_multicast_handler(sdev);
        ...
        work = kmalloc(sizeof(struct asgard_leader_pkt_work_data), GFP_KERNEL);
        work->sdev = sdev;

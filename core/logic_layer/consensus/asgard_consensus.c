...
void dump_consensus_throughput(struct asgard_logger *slog)
{// Write Start Timestamp:
    write_log(slog, 1, slog->first_ts);
    asgard_dbg("Consensus Throughput first TS: %lld\n", slog->first_ts);
    // Write End Timestamp:
    write_log(slog, 2, slog->last_ts);
    asgard_dbg("Consensus Throughput last TS: %lld\n", slog->last_ts);
    // Write throughput:
    write_log(slog, 3, slog->applied);
    asgard_dbg("Consensus Throughput applied: %d\n", slog->applied);
}
